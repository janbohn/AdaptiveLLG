...
def threshold(eta_h, lower, upper):
    """Threshold marking strategy"""
    mesh = eta_h.function_space().mesh()
    V = eta_h.function_space()
    dofmap = V.dofmap()
    ...
    markers = MeshFunction("bool", mesh, mesh.geometry().dim(), False)
    marked = np.zeros_like(etas.array(), dtype=np.bool)
    marked[np.where(etas.array() > upper)] = True
    markers.set_values(marked)
    
    dorefine=np.count_nonzero(marked) >0
    return np.count_nonzero(etas.array() < lower)/eta_h.function_space().dim() ,markers , dorefine
